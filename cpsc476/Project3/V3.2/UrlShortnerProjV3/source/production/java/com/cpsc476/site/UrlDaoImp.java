...
public class UrlDaoImp extends JdbcDaoSupport implements UrlDaoInterface {
	...
	public void insertoneurl(String username, String lurl, String surl, int clicks) {
		getJdbcTemplate().update("insert into project2.url(username,longurl,shorturl,clicks)values (?,?,?,?)", username,
				lurl, surl, clicks);
	...
	public List<Url> findAllurl(String username) {
		return getJdbcTemplate().query(
				"select distinct m.longurl ,m.shorturl , (select sum(clicks)  from project2.url where shorturl = m.shorturl)as totalclicks from project2.url as m where username = ? ",
	...
	public Url checkforUrl(String username, String longurl) {
		return getJdbcTemplate().queryForObject(
				"Select count(longurl) as counts from project2.url where longurl = ? and username = ?",
				new urlcountMapper(), longurl, username);
	}

	@Override
	public Url checkifUrlExists(String shorturl) {
		return getJdbcTemplate().queryForObject("Select count(longurl) as counts from project2.url where shorturl = ?",
				new urlcountMapper(), shorturl);
	}

	@Override
	public Url getlongurl(String url) {
		return getJdbcTemplate().queryForObject("select distinct longurl from project2.url where SHORTURL = ?",
				new longurlMapper(), url);
	}

	@Override
	public void updateclicks(String surl, String username) {
		getJdbcTemplate().update("UPDATE project2.url set clicks = clicks+1  where shorturl = ? and username = ?", surl,
				username);
	}
